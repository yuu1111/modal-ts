...
test("ClsCall", async () => {
	const cls = await Cls.lookup("modal-ts-test-support", "EchoCls");
	...
	const cls2 = await Cls.lookup("modal-ts-test-support", "EchoClsParametrized");
...
test("ClsNotFound", async () => {
	const cls = Cls.lookup("modal-ts-test-support", "NotRealClassName");
	await expect(cls).rejects.toThrowError(NotFoundError);
});

test("ClsCallInputPlane", async () => {
	const cls = await Cls.lookup("modal-ts-test-support", "EchoClsInputPlane");
	const instance = await cls.instance();
